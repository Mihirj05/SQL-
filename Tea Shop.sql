...
SELECT 
    ...
    ts.state,
    ts.country,
    SUM(s.quantity * m.price) AS total_revenue
FROM Sales s
JOIN Tea_Shops ts ON s.shop_id = ts.shop_id
JOIN Menu_Items m ON s.item_id = m.item_id
GROUP BY ts.shop_id, ts.shop_name, ts.city, ts.state, ts.country;
